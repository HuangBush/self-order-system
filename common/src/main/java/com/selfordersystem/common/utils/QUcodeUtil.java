 ...
 * @since 
 */
public final class QUcodeUtil extends LuminanceSource {
 
    private static final Logger logger = LoggerFactory.getLogger(QUcodeUtil.class);
    ...
    public QUcodeUtil(BufferedImage image) {
        this(image, 0, 0, image.getWidth(), image.getHeight());
    }
 
    public QUcodeUtil(BufferedImage image, int left, int top, int width, int height) {
        super(width, height);
    ...
    public LuminanceSource crop(int left, int top, int width, int height) {
        return new QUcodeUtil(image, this.left + left, this.top + top, width, height);
    }
    ...
    public LuminanceSource rotateCounterClockwise() {
        ...
        g.dispose();
        int width = getWidth();
        return new QUcodeUtil(rotatedImage, top, sourceWidth - (left + width), getHeight(), width);
    }
    ...
    public static String parseQRCode(String filePath) {
        ...
        try {
            ...
            LuminanceSource source = new QUcodeUtil(image);
    ...
    public static void main(String[] args) {
        String text = "http://192.168.43.112:8082/food/clientLogin.action?d_id=1001&d_password=123456"; //一号桌登录
